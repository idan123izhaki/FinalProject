...
//    uint32_t input_size = serializedData.size();
//
//
...
std::vector<uint8_t> createVector(uint32_t num1, uint32_t num2, std::vector<uint8_t> data, size_t dataSize) {
...
int main() {
    // Example uint32_t number
    std::uint32_t number1 = 1; // example value
    std::uint32_t number2 = 2; // example value
    std::string data = "hello world!";
    std::cout << "THE NUMBER1 IS: " << number1 << std::endl;
    std::cout << "THE NUMBER2 IS: " << number2 << std::endl;
    //std::string result = convertIntToString(number);
    //std::cout << "THE OUTPUT STRING FROM THE MAIN FUNCTION IS:\n" << result << std::endl;
    std::vector<uint8_t> input(data.begin(), data.end());

    std::vector<uint8_t> result = createVector(number1, number2, input, input.size());
    uint32_t first_number, second_number;
    std::memcpy(&first_number, result.data(), sizeof(uint32_t));
    std::memcpy(&second_number, result.data() + sizeof(uint32_t), sizeof(uint32_t));
    std::cout << "THE FIRST NUMBER IS: " << first_number << std::endl;
    std::cout << "THE FIRST NUMBER IS: " << second_number << std::endl;
    return 0;
}
